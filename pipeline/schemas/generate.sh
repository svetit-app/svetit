...
SAVED_HASH_PATH="$DIR_PATH/.hash"
SAVED_HASH=$(cat "$SAVED_HASH_PATH" 2>/dev/null)

GENERATED_HASH=$(find "$DIR_PATH" -type f ! -name '.hash' | xargs -I "{}" sha1sum "{}" | sha1sum | head -c 40)

[ "$GENERATED_HASH" = "$SAVED_HASH" ] && exit 0
...
docker run --rm -v "$DIR_PATH":/spec -v "$TMP_PATH":/tmp redocly/cli bundle -o "$API_YAML_PATH_GUEST" ./openapi.yaml
...
echo -n "$GENERATED_HASH" > "$SAVED_HASH_PATH"
