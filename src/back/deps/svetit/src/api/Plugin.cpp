...
#include "Plugin_parsers.ipp"

namespace svetit {
namespace api {

bool operator==(const svetit::api::Plugin& lhs,
                const svetit::api::Plugin& rhs) {
  return lhs.id == rhs.id && lhs.projectId == rhs.projectId &&
         lhs.name == rhs.name && lhs.description == rhs.description &&
         lhs.key == rhs.key && true;
}
...
USERVER_NAMESPACE::formats::json::Value Serialize(
  ...
  if (value.id) {
    vb["id"] = USERVER_NAMESPACE::chaotic::Primitive<
        std::int64_t,
        USERVER_NAMESPACE::chaotic::Minimum<svetit::api::Plugin::kIdMinimum>>{
        *value.id};
  }
  ...
  vb["key"] = USERVER_NAMESPACE::chaotic::Primitive<
      std::string, USERVER_NAMESPACE::chaotic::MinLength<1>,
      USERVER_NAMESPACE::chaotic::MaxLength<128>>{value.key};
