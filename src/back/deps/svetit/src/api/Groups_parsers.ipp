...
static constexpr USERVER_NAMESPACE::utils::TrivialSet
    ksvetit__api__Groups_PropertiesNames = [](auto selector) {
      return selector().template Type<std::string_view>().Case("list").Case(
          "total");
    };

template <typename Value>
svetit::api::Groups Parse(
    Value value, USERVER_NAMESPACE::formats::parse::To<svetit::api::Groups>) {
  ...
  res.total = value["total"]
                  .template As<USERVER_NAMESPACE::chaotic::Primitive<
                      std::int64_t, USERVER_NAMESPACE::chaotic::Minimum<
                                        svetit::api::Groups::kTotalMinimum>>>();
