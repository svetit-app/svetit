...
export class SchemeGroupsComponent implements OnInit {
	...
	constructor(private schemes: SchemesService, private users: UsersService) {
	}
	...
	addUserToSchemeGroup($event: Scheme_Group & { role?: Group_User_Roles }) {
		this.schemes.addUserToSchemeGroup($event.id, this.users.user.id, $event.role)
			.subscribe(() => this.fetchGroups());
	}

	removeUserFromSchemeGroup($event: Scheme_Group) {
		this.schemes.removeUserFromSchemeGroup($event.id, this.users.user.id)
			.subscribe(() => this.fetchGroups());
	}

	private fetchGroups() {
		this.schemes.getSchemeGroupsForUser(this.users.user.id)
